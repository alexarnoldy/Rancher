...
```
NOTE: It is normal to see an error that says "-bash: kubectl: command not found"
...
Pick a K8s release (e.g. v1.33) of RKE2 from: https://github.com/rancher/rke2/releases and https://www.suse.com/suse-rancher/support-matrix/all-supported-versions/
...
```bash
sudo bash -c "curl -sfL https://get.rke2.io | INSTALL_RKE2_CHANNEL="${INSTALL_RKE2_CHANNEL}" sh -"
```
...
= Optionally, add nodes only after RKE2 is installed and running correctly:
...
NOTE: If your Rancher Management Server is accessible from the Internet, you can use External, Rancher-managed HTTP-01 TLS certificates or Let's Encrypt certificates using DNS-01 challenge certifictes. If your Rancher Management Server is not accessible from the Internet, you must use External, Let's Encrypt certificates using DNS-01 challenge (requires API tokens for the DNS service), or Rancher-managed private certficates.

IMPORTANT: Select ONLY ONE of the following options: 1) "Let's Encrypt publicly trusted certificates using DNS-01 challenge", 2) "External (Bring Your Own) Certificates", 3) "Rancher-managed private or HTTP-01 public TLS certificates" for TLS certificate management appropriate for your installation. See https://ranchermanager.docs.rancher.com/getting-started/installation-and-upgrade/install-upgrade-on-a-kubernetes-cluster#3-choose-your-ssl-configuration for more information.

=== 1) Let's Encrypt publicly trusted certificates using DNS-01 challenge:

* Follow these instructions to install cert-manager from the Rancher Application Collection (https://apps.rancher.io): https://gist.github.com/alexarnoldy/2ed5d41f77738609a468952e30c42d43

* Create a public DNS record for your server, resolved to its private network IP address. 

* Set the FQDN for the server:

```bash
export SERVER_FQDN="server.yourdomain.com"
```
...
spec:
  secretName: tls-rancher-ingress # The secret name that Rancher expects
...
=== 2) External (Bring Your Own) Certificates:
...
=== 3) Rancher-managed private or HTTP-01 public TLS certificates:
...
* Install cert-manager from the Rancher Application Collection (https://apps.rancher.io): 

** Create the cert-manager namsespace: `kubectl create ns cert-manager`

** Create Service Account tokens or Access tokens

** Use the AppCo examples to:

*** Create the docker-registry secret in the cert-manager namespace

*** Helm login to AppCo (before doing the cert-manager helm install)

** Check for a current version of cert-manager (1.20.2 in this example) at: <https://apps.rancher.io/applications/cert-manager>

```bash
export CERT_MANAGER_VERSION="1.20.2"
```

Install the cert-manager chart from AppCo:
...
=== Select between community-support Rancher and Rancher-Prime. 

CAUTION: Use only one of the following Helm repo set up procedures:

** FOR COMMUNITY-SUPPORTED RANCHER ONLY: Set the following variable to `rancher-latest` or `rancher-stable`:

```bash
RANCHER_REPO=""
```
*** Add the community-supported Rancher repo:

```bash
REPO_SHORT_NAME=$(echo ${RANCHER_REPO} | awk -Francher- '{print$2}')
helm repo add ${RANCHER_REPO} https://releases.rancher.com/server-charts/${REPO_SHORT_NAME}
```
...
```bash
REPLICAS=$(kubectl get nodes | grep -E 'control-plane|master' | wc -l)
SET_DEVEL_FLAG=""     # Set to --devel for access to development features
RANCHER_CHART_VERSION=""    # CHART VERSION not APP VERSION
TLS_SOURCE=""         
# Set TLS_SOURCE to:
# "--set ingress.tls.source=rancher" for Rancher-managed Private Certificates, 
# "--set ingress.tls.source=letsEncrypt" for Rancher-managed HTTP-01 Let's Encrypt Certificates,
# "--set ingress.tls.source=secret" for External (BYO) Certificates or Let's Encrypt DNS-01 challenge Certificates
```
...
```bash
echo; echo "RANCHER_REPO="${RANCHER_REPO}; echo "RANCHER_CHART_VERSION="${RANCHER_CHART_VERSION}; echo "SERVER_FQDN="${SERVER_FQDN}; echo "BOOTSTRAPPW="${BOOTSTRAPPW}; echo "TLS_SOURCE="${TLS_SOURCE}; echo "SET_DEVEL_FLAG="${SET_DEVEL_FLAG}; echo "REPLICAS="${REPLICAS}
```
...
Log into the Rancher web UI at ${SERVER_FQDN} and the user `admin` with a password of ${BOOTSTRAPPW}.
