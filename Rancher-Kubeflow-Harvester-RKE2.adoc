...
* Ensure all pods have a `STATUS` of `Running` and all of the containers in each pod are running:
```sh
for EACH in auth cert-manager istio-system knative-eventing knative-serving kubeflow; do kubectl get pods -n ${EACH}; read -p "<Enter to continue>"; echo ""; done
```
