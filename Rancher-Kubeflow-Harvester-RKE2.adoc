...
DNSZONE="" # E.g. "suse.com"
FQDN="" # E.g. "kubeflow.suse.com"
EMAIL_ADDR="" # valid email address for the Let's Encrypt certificate
...
spec:
  acme:
    ...
    privateKeySecretRef:
      name: letsencrypt-istio-tls-cert # Where the tls.key and tls.crt of the generated cert will be stored
    solvers:
    - selector:
        dnsZones: 
          - "${DNSZONE}"
...
```sh
kubectl create -n kubeflow secret generic route53-credentials-secret --from-literal=secret-access-key=${AWS_SECRET_ACCESS_KEY}```

* Verify the contents of the secret:
```sh
kubectl get -n kubeflow secret route53-credentials-secret -o jsonpath={.data.secret-access-key} | base64 -d; echo ""
```

## Update dex to allow the Let's Encrypt DNS01 challenge:
...
spec: 
  secretName: kubeflow-certificate-secret
  commonName: ${FQDN}
  dnsNames:
    - ${FQDN}
  issuerRef:
