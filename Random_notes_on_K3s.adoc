...
sudo podman run --name k3s-mariadb --pod k3s-mariadb-pod -v /opt/k3s-mariadb:/var/lib/mysql:Z -e  MYSQL_ROOT_PASSWORD="${MDB_ROOT_PASS}" -e MYSQL_USER="${MDB_USER_NAME}" -e MYSQL_PASSWORD="${MDB_USER_PASS}" -d docker.io/library/mariadb
----
...
mariadb -h ${MDB_VIP} -u root -p${MDB_ROOT_PASS}

create database k3s;

grant all privileges on k3s.* to 'k3s'@'%'  identified by 'k3sPassw0rd';
...
----
curl -sfL https://get.k3s.io | K3S_DATASTORE_ENDPOINT='mysql://${MDB_USERNAME}:${MDB_PASSWORD}@tcp(${MDB_VIP}:3306)/k3s' INSTALL_K3S_VERSION=${K3s_VERSION} INSTALL_K3S_EXEC='server --write-kubeconfig-mode=644 ${LOCAL_PATH_SC}' sh -s -
----
