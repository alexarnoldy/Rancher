...
** `kubectl.exe get clusters.management.cattle.io`
* Remove by patching the finalizers with an empty array []:
** export EACH=""
** `kubectl -n fleet-default patch clusters ${EACH} -p '{"metadata":{"finalizers":[]}}' --type=merge`
** `kubectl patch clusters.management.cattle.io ${EACH} -p '{"metadata":{"finalizers":[]}}' --type=merge`
